...
public class Gap {
    ...
    public int maxGap(int[] A, int n) {
        ...
        java.util.ArrayList<Integer>[] bucket = new java.util.ArrayList[n+1];
        ...
        for (int i = 1; i < bucket.length; i++) {
        	int lastMax = curMax;
        	if (bucket[i].size() != 0) {
        		updateMinAndMax(bucket[i]);
        		int gap = curMin - lastMax;
        		if (gap > gapMax)
        			gapMax = gap;
        	}
        }
    ...
    // list 非空
    public void updateMinAndMax(ArrayList<Integer> list) {
    	int min = list.get(0);
    	int max = list.get(0);
